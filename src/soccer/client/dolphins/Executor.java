...
package soccer.client.dolphins;
...
public class Executor
{
  ...
  private void shootGoal() throws IOException
  {
    double dir = world.see.player.position.direction(world.goal);
     double dir2 = world.see.player.position.direction(world.door);
    double dir3=    Math.abs(Math.abs(dir) - Math.abs(dir2))/180;
    double value = dir +dir3;
    System.out.println(dir);
    System.out.println(dir3);
    kicker = new KickData( ( (dir+value)/2 ), 100);
    command = new Packet(Packet.KICK, kicker, AIPlayers.address, AIPlayers.port);
    transceiver.send(command);
  }
...
private void dribbleTo() throws IOException
  {
    
  double dirdribble = world.see.player.position.direction(world.goal);
      driver = new DriveData(dirdribble, 100);
      command = new Packet(Packet.DRIVE, driver, AIPlayers.address, AIPlayers.port);
      transceiver.send(command);
  }  
  public void executing() throws IOException
  {
    switch(world.actionType)
    {
     ...
     case World.DRIBBLE: dribbleTo(); break;
    }
  }

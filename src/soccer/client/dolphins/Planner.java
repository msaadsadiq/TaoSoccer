...
public class Planner
{
  ...
  private void kicking()
  {
    ...
    if(world.see.player.position.distance(world.goal) < 60) world.actionType = World.SHOOT;
	...
	xx = world.see.player.position.getX() + 15 * Math.cos(Util.Deg2Rad(pass_dir));
	yy = world.see.player.position.getY() + 15 * Math.sin(Util.Deg2Rad(pass_dir));

	if ( xx > World.LENGTH/2 || xx < -World.LENGTH/2 
	     || yy > World.WIDTH/2 || yy < -World.WIDTH/2 ) value -= 526;
	...
	value += (your_mean_time - my_mean_time) * 1;
	value += (your_best_time - my_best_time) * 5;

	// move the ball to opporent side is better
	if(world.see.player.side == 'l')
	  value += 140 * (-0.5 + (180 - Math.abs(pass_dir))/180);
	else value += 140 * (-0.5 + (Math.abs(pass_dir))/180);

	// I'd like to kick to opporent's goal
        goal_dir = world.see.player.position.direction(world.goal);
	value += 140 * (-0.5 + (Math.abs(Math.abs(goal_dir) - Math.abs(pass_dir)))/180);

	// I'd like to avoid my own goal
        door_dir = world.see.player.position.direction(world.door);
	value += 140 * (-0.5 + (Math.abs(Math.abs(door_dir) - Math.abs(pass_dir)))/180);
